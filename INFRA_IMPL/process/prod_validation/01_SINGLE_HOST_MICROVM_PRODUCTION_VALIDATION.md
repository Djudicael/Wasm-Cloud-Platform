...
| Testbed kernel compatibility (historical) | SUPERSEDED TEST FIXTURE | The tinyconfig/6.1.80 artifact lacked features needed to model the platform and eBPF reliably. It was replaced by the reproducible 6.18.48/Firecracker 1.16.1 testbed compatibility unit. Neither kernel is a platform production artifact. |
...
| Rehearsal provenance snapshot | PARTIAL | Source commit is `4abbc7db5087862de0331f03a2b7904433dba727` with a dirty worktree intentionally preserved. Rust is pinned to 1.97.1. SHA-256 values were recorded for the platform lock and binaries, WASI applications, and testbed-only kernel/rootfs fixtures. A standards-based SBOM and clean platform promotion build are still missing. The testbed assets are not platform release subjects. |
...
#### P10-07 clock validation (fresh schema-5 microVM pass)

On 2026-09-01 the local PostgreSQL fixture advanced to image schema 5. It keeps
schema 4's fail-closed initial Chrony synchronization, configures independent
Cloudflare and Google local-test sources, and emits initial plus 30-second
tracking/source snapshots to the recorded VM serial log. The canonical 1-GiB
image rebuilt successfully, reports schema 5, contains both sources and the
tracking observer, and passes read-only `e2fsck`.

The disposable observability stack now exports the database wall clock as
`pg_clock_epoch_seconds` and loads `PostgreSQLClockSkewHigh` plus
`PostgreSQLClockMetricMissing`, and `PostgreSQLTimeSourceUnavailable`. A minimal
guest endpoint reports synchronized state, usable-source count, and sample age;
Promtool validates all 35 expressions across five alert-rule files. The pinned
local exporter accepts the query file but warns that
its extended-query option is deprecated; production must use a maintained SQL
exporter or managed integration for the equivalent metric.

`scripts/vm/validate-postgres-clock.sh` verifies image identity, selected-source
and synchronized-state serial evidence, direct and Prometheus-observed offset,
the missing-metric alert during an exact recorded-VM `SIGSTOP`, alert resolution,
fresh Chrony tracking after `SIGCONT`, and bounded post-resume skew. The disaster-
recovery runner now fails when source database skew exceeds its configured
five-second limit instead of permanently reporting the obsolete schema-3 clock
limitation.

The fresh run started on 2026-09-01 and completed on 2026-09-02. The final
three-node topology used a separate NATS VM and schema-5 PostgreSQL VM. A
90-second exact-VMM suspension produced explicit unsynchronized/source-zero
metrics. The observer reset stale source measurements once, issued an
authenticated forced step and burst, and recovered with two usable sources in
about 25 seconds. The canonical validator passed with 0.444 seconds direct skew
before the pause, 1.522 seconds Prometheus-observed skew, and 1.151 seconds
direct skew after recovery. Missing-clock-metric and unavailable-time-source
alerts both fired and resolved; the skew alert passed deterministic promtool
coverage and was clear after recovery.

The live alert gate passed all 35 rules, 28 always-present metric names, seven
firing/resolved delivery pairs, and deduplication. `send_failed` and
`enqueue_failed` OpenTelemetry counters are event-created and may legitimately
be absent in a healthy collector. The final encrypted OIDC recovery rehearsal
verified 26 public tables, 11/11 migration records, and six focused Playwright
login/dashboard checks. It measured 1.316 seconds for backup, 6.928 seconds for
database restore, 44.844 seconds to application readiness, 53.154 seconds for
the full journey, and 1.78 seconds source-database clock skew. Cleanup restored
the original two routes/applications and removed the recovery container. Final
OIDC readiness reported `database=ok`, Chrony reported two usable sources, all
three nodes were healthy, and Prometheus had no active alerts. Evidence is in
`evidence/2026-09-01-single-host/P10-07-postgres-clock/`.

The run also found and retained failed-attempt evidence for a missing HTTP
applet, guest ownership errors, Prometheus scrape-protocol compatibility,
malformed exporter options, stale synchronization after a long host pause,
unauthorized Chrony commands, invalid command-socket directory ownership, and
repeated source resets that prevented convergence. These observations drove the
final source changes instead of being discarded after the passing run.

The public sources and local database are test fixtures, not a platform
production architecture. Production requires at least three independent,
operator-controlled sources, NTS or equivalent authenticated private time where
