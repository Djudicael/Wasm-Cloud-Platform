...
| P10-02 | P1 | Production secret lifecycle is not implemented end to end. Local credentials are placeholders and the development seeder has printed test passwords/tokens. | External secret-manager integration, non-exportable key policy where applicable, rotation/revocation drill, log-redaction tests, and admission that rejects local defaults. |
