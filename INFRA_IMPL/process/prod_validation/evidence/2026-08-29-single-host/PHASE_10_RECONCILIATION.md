...
| Platform release legitimacy | **GO TO SIGNED RELEASE CANDIDATE** | The supported source contract may be released after the exact candidate is built and attested by the release workflow. A second physical host is not a platform release prerequisite. |
...
exercise. Local evidence cannot prove the identity of a future release artifact
or the configuration of a future production environment, so those decisions are
made separately for the exact release and deployment profile.

## Ownership matrix

| Gate | Platform result | Remaining evidence owner |
|---|---|---|
| P10-01 release supply chain | Source and local tamper/reproducibility tests pass. | Release owner: create an approved semantic-version tag, retain immutable digests and attestations, and independently verify downloaded bytes. |
...
1. Cut the approved semantic-version tag from the reviewed source revision.
2. Let the release workflow build, attest, and publish the immutable candidate;
   do not promote manually built binaries or mutable microVM images.
3. Independently verify the candidate digest, provenance, SBOM, and signatures.
4. Deploy that same digest to production-equivalent staging using the operator's
   host automation, PKI, NATS, secrets integration, telemetry path, alert
   receiver, DNS, load balancer, and resource limits.
5. Run only the applicable platform runbooks against every node class and fault
...
6. If the deployment claims host-failure tolerance or cross-host availability,
...
7. Record approvers, time-bounded exceptions, rollback authority, and the exact
   evidence links in the platform production deployment checklist.
8. Issue a new `GO` or `NO-GO` for that exact digest and environment. This local
