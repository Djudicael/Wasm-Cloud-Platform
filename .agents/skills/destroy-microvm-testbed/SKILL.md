...
4. If a `.services.json` companion exists, the script validates and removes only its exact state-labeled Podman observability containers and state-derived runtime directory, then validates and stops the recorded HAProxy PID and removes its generated config and log.
