...
description: Audit and synchronize all maintained Wasm Cloud Platform documentation, including the primary user and operator manual under docs/, with the current code, manifests, scripts, CI, security policy, deployment model, and validated behavior. Use after code, dependency, toolchain, configuration, operational workflow, or production-validation changes, or when project documentation may be stale.
---
...
3. Prefer one canonical source with links from other documents. Avoid copying
   volatile dependency tables or long procedures into multiple files.
