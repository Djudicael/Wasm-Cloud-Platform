...
mod tests {
    ...
    async fn test_upstream_registry_round_robin() {
        ...
        registry
            .add(&app_id, UpstreamEndpoint { addr: addr1, h2c: false })
            .await;
        registry
            .add(&app_id, UpstreamEndpoint { addr: addr2, h2c: true })
            .await;
        registry
            .add(&app_id, UpstreamEndpoint { addr: addr3, h2c: false })
            .await;
        ...
        assert_eq!(
            ...
            Some(UpstreamEndpoint { addr: addr1, h2c: false })
        );
        assert_eq!(
            registry.next(&app_id).await,
            Some(UpstreamEndpoint { addr: addr2, h2c: true })
        );
        assert_eq!(
            registry.next(&app_id).await,
            Some(UpstreamEndpoint { addr: addr3, h2c: false })
        );
        assert_eq!(
            registry.next(&app_id).await,
            Some(UpstreamEndpoint { addr: addr1, h2c: false })
    ...
    async fn test_upstream_registry_remove_and_empty() {
        ...
        // Add and verify
        registry
            .add(&app_id, UpstreamEndpoint { addr: addr1, h2c: true })
            .await;
        assert_eq!(
            registry.next(&app_id).await,
            Some(UpstreamEndpoint { addr: addr1, h2c: true })
        );
