...
use pkcs8::{der::Decode, PrivateKeyInfoRef};
...
pub fn generate_rsa_key() -> GeneratedRsaKey {
    ...
    let private_key = PrivateKeyInfoRef::from_der(pkcs8.as_ref())
        .expect("parse generated PKCS#8 key")
        .private_key;
    let public = PublicKeyComponents::<Vec<u8>>::from(key_pair.public_key());

    GeneratedRsaKey {
        encoding_key: EncodingKey::from_rsa_der(private_key.as_bytes()),
