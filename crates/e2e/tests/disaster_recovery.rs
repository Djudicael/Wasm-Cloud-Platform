...
async fn test_corrupted_routes_rebuild_from_jetstream() {
    ...
    // Add multiple routes
    add_route(&bus, "route1.local", app_id).await.expect("Failed to add route1");
    add_route(&bus, "route2.local", app_id).await.expect("Failed to add route2");
    add_route(&bus, "route3.local", app_id).await.expect("Failed to add route3");
    ...
    assert_eq!(response2.status(), 200, "Route should work after partial rebuild");

    let response3 = send_request(node2.proxy_port, "route2.local", "/")
        .await
        .expect("Failed to send request");
    assert_eq!(response3.status(), 200, "Route2 should work after partial rebuild");
...
async fn test_total_node_loss_recovery() {
    ...
    let artifact_url = format!("http://127.0.0.1:{}/artifacts/{}", node1.artifact_port, sha256);
